...
const AccountPage = () => {
  ...
  const fetchUserImages = async (token) => {
    try {
      ...
      console.log("Base URL raw:", JSON.stringify(api.defaults.baseURL));
      const profileResponse = await api.get(`/api/user/profile-picture`, {
        headers: {
          'authorization': token,
        },
      });
      ...
      const backgroundResponse = await api.get(`/api/user/background-picture`, {
        headers: {
          'authorization': token,
        },
      });
  ...
  const uploadBackgroundPicture = async () => {
    ...
    try {
      ...
      const response = await api.post(`/api/user/upload-background-picture`, formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
          'authorization': token,
        },
  ...
  return (
    ...
    >
      ...
      <View style={{ position: 'relative', flex: 1 }}>
        <Image
          source={backgroundPictureUrl ? { uri: `http://10.0.0.85:5001/uploads/${backgroundPictureUrl}` } : { uri: backgroundImageUri }}
          resizeMode="cover"
          style={{
            height: 228,
            width: "100%",
      ...
      <View style={{ flex: 1, alignItems: "center", }}>
        <Image
          ...
          source={profilePictureUrl ? { uri: `http://10.0.0.85:5001/uploads/${profilePictureUrl}` } : { uri: profileImageUri }}
          resizeMode="contain"
        />
